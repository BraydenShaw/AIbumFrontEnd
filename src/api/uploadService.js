...
export const uploadService = {
   ...
   * @param {FormData} formData - FormData object containing the file. The file should be added with the key 'file'.
   * @param {Function} onUploadProgress - Callback function to listen for upload progress
   * @returns {Promise<any>}
   */
  async upload(formData, onUploadProgress) {
    // Backend upload API URL, now using the proxied path
    const uploadUrl = '/api/debug/images'; // Changed to use the proxy path
    ...
    try {
      ...
      const response = await axios.post(uploadUrl, formData, {
        ...
        onUploadProgress: (progressEvent) => {
          if (onUploadProgress) {
            // progressEvent contains loaded (bytes uploaded) and total (total bytes)
            const percentCompleted = Math.round((progressEvent.loaded * 100) / progressEvent.total);
            onUploadProgress({ loaded: progressEvent.loaded, total: progressEvent.total, percent: percentCompleted });
          }
        },
      ...
      // Check axios response status code (axios automatically treats 2xx status codes as success)
      if (response.status >= 200 && response.status < 300) {
        console.log('File uploaded successfully:', response.data);
        return response.data; // Return data from the backend response
      } else {
        // For non-2xx status codes, axios generally throws an error, but here we check explicitly
        const errorMessage = response.data.message || `Upload failed, status code: ${response.status}`;
        console.error(`Upload failed: ${errorMessage}`, response);
        throw new Error(errorMessage);
      }
    } catch (error) {
      // Catch network errors, timeouts, or non-2xx status codes from backend (axios treats these as errors)
      if (axios.isAxiosError(error)) {
